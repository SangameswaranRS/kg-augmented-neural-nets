...
newsgroups_test_data_sequence_unpadded = sequence_tokenizer.texts_to_sequences(newsgroups_test_data)
print('[INFO] Padding Sequences')
newsgroups_test_data_sequence_padded = pad_sequences(newsgroups_test_data_sequence_unpadded, maxlen=MAX_SEQUENCE_LENGTH)
...
texts = []
i = 0
for seq in newsgroups_test_data_sequence_padded:
    i=i+1
    curr_word_list = []
    for num in seq:
        if num == 0:
...
print(d4_processed.shape)
print('[INFO] Processing d5')
d5_input = d4_processed.reshape(7532,299,2,10,1)
d5_input_file = open('d5_input_test','wb')
