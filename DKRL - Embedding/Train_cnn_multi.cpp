...
int relation_num=0,entity_num=0,word_num=0;
...
void sgd()		//SGD training
{
	...
	for (int epoch=0; epoch<nepoch; epoch++)
	{
		...
		cout<<"epoch:"<<epoch<<" res_triple  "<<res_triple<< "  res_cnn  " << res_cnn << "  res_c_e  ";
		cout << res_c_e << "   res_e_c  " << res_e_c << "   res_normal  " << res_normal << endl;
		//output
		FILE* f2 = fopen(("relation2vec."+version).c_str(),"w");
		FILE* f3 = fopen(("entity2vec."+version).c_str(),"w");
		FILE* f4 = fopen(("word2vec."+version).c_str(),"w");
		FILE* f5 = fopen(("weight1."+version).c_str(),"w");
		FILE* f6 = fopen(("weight2."+version).c_str(),"w");
...
void prepare()
{
	...
	FILE* f3 = fopen("word2id.txt","r");
	FILE* f4 = fopen("entityWords.txt","r");
	char buf[1000]={};
	int x = 0;
	cout<<"build entity2ID"<<endl;
	...
	while (fscanf(f1,"%s %d\n",buf,&x) == 2)
	{
		...
		string st=buf;
		int t = x;
		entity2id[st]=t;		//<entity,ID>
		id2entity[t]=st;		//<ID,entity>
		entity_num++;
	}
	cout<<"entity_num"<<entity_num;
	cout<<"build relation2Id"<<endl;
	...
	while (fscanf(f3,"%s%d\n",buf,&x)==2)
	{
		...
		string st=buf;
		int t = x;
		word2id[st]=t;		//<word,ID>
		id2word[t]=st;		//<ID,word>
		word_num++;
	}
	cout<<"build entityword"<<endl;
